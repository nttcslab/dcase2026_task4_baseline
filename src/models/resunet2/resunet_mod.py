...
class ResUNet30MultiPredict(ResUNet30):
    def __init__(self, input_channels, output_channels, target_sources_num, label_len):
        ...
        self.nsources = target_sources_num
    
    def forward(self, input_dict):
        with torch.no_grad(): # this model is just for inference
            bs, lblen = input_dict['label_vector'].shape # [bs, label_len x nsources]

            assert (lblen == self.label_len * self.nsources)
            label_vector = input_dict['label_vector']
            with torch.no_grad():
                copylb = label_vector.clone()
                source_mask = copylb.view(bs, self.nsources, self.label_len).sum(dim=2)
                source_mask = (source_mask!=0).to(label_vector.dtype)
                source_mask = source_mask.reshape(bs * self.nsources, 1)
            label_vector = label_vector.reshape(bs, self.nsources, self.label_len)
            label_vector = label_vector.reshape(bs * self.nsources, self.label_len)
            mixtures = input_dict['mixture']
            mixtures_repeat = mixtures.repeat_interleave(self.nsources, dim=0)
            conditions = self.label_embedding(label_vector)
            film_dict = self.film(
                conditions=conditions,
            )
            ...
            output_dict['waveform'] = output_dict['waveform'].reshape(bs, self.nsources, 1, mixtures.shape[-1])
