...
class FinalJointPITLoss(nn.Module):
    def __init__(self, n_classes=18, weights=None):
        super().__init__()
        # 원래 클래스 0~(n_classes-1)에 더해, 빈 슬롯을 위한 '침묵(Silence)' 클래스를 n_classes번으로 명시
    ...
    def forward(self, preds, targets):
        ...
        preds: {
            'waveforms': [B, K, T],
            'class_logits': [B, K, n_classes + 1],  # n_classes개 실제 클래스 + 1개 silence
            'doa_pred': [B, K, 3]
        }
        targets: {
            'waveforms': [B, K, T],
            'labels': [B, K],            # 빈 슬롯은 n_classes(침묵)로 전처리되어 들어와야 함
        ...
        for t_idx in range(K):
            ...
            for p_idx in range(K):
                ...
                c_ce = F.cross_entropy(preds['class_logits'][:, p_idx, :], 
                                       ...
                                       reduction='none').detach() * self.weights['ce']  # Fix: detach for permutation search only
